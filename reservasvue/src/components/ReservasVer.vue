			...
			<table v-if="reservas.length > 0">
				...
				<tbody>
					<tr v-for="reserva in reservas" :key="reserva.id">
						...
						<td>
							<router-link to="/Pasarelas" v-if="!reserva.pagada" @click="pagarReserva(reserva.id)"
								class="action-button pagar">
								💳 Pagar
							</router-link>

							<span v-else class="estado-pagado">✔ Pagada</span>
							<button @click="eliminarReserva(reserva.id)" class="action-button delete">🗑</button>
						</td>
					</tr>
				</tbody>
			</table>
